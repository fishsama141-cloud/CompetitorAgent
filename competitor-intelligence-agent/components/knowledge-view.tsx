...
export function KnowledgeView() {
  ...
  async function runSearch() {
    ...
    try {
      const res: SearchResponse = await searchKnowledge({
        query, top_k: topK, competitor_id: competitorId,
  ...
  function addFiles(list: FileList | null) {
    ...
    next.forEach(async (f) => {
      try { await uploadDocument({ competitor_id: competitorId, file_name: f.name, document_type: 'changelog' }) } catch { /* fallback */ }
    })
  ...
  async function send() {
    ...
    setDraft('')
    setMessages((prev) => [...prev, { id: `msg_${prev.length + 1}`, role: 'user', content: text }])
    setThinking(true)
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `msg_${prev.length + 1}`, role: 'assistant',
          content: '基于知识库检索，竞品在高峰期的稳定性问题是最可复用的切入点[chunk_002]；同时其开放平台的批量推理定价正在重塑企业侧价格锚点[chunk_003]。建议把「不中断的长会话」作为差异化叙事，并在企业方案中补齐审计与权限能力。',
          citations: [
            { chunk_id: 'chunk_002', source_title: 'App Store 评论', raw_text_snippet: '经常提示网络连接超时，需要反复重发才能拿到结果。' },
            { chunk_id: 'chunk_003', source_title: 'Changelog', raw_text_snippet: '开放平台上线深度 API 与批量推理定价，面向企业客户提供私有化部署。' },
          ],
        },
      ])
    } finally {
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white via-violet-50/60 to-violet-100/40 px-6 py-10 lg:px-10 lg:py-14">
        <div className="relative z-10 flex flex-wrap items-start justify-between gap-6">
          <div className="flex flex-col gap-2">
            <span className="font-mono text-[10px] font-medium tracking-[0.2em] text-primary/60 uppercase">Vector Store · RAG</span>
            <h1 className="text-[26px] font-bold leading-tight tracking-tight lg:text-[32px]">知识库 & RAG 检索</h1>
            <p className="max-w-lg text-[13px] leading-relaxed text-muted-foreground">
              语义搜索与 AI 驱动的智能问答。上传文档构建向量索引，用自然语言查询竞品情报并获取带引用来源的回答。
            </p>
          </div>
          {/* Metrics in hero */}
      ...
      {/* ================================================================
          SECTION 2 — Two Column: Search + Upload | Chat
          ================================================================ */}
      <section className="mt-8">
        <div className="grid gap-8 xl:grid-cols-[1fr_1fr]">
                ...
                <CardContent className="flex flex-col gap-4 pt-6">
                  <button
                    ...
                    className={cn(
                      ...
                      dragging ? 'border-primary/50 bg-primary/[0.04]' : 'border-border bg-muted/20 hover:border-primary/30 hover:bg-primary/[0.02]',
                    )}
                  >
                  ...
                  {files.length > 0 && (
                    <ul className="flex flex-col gap-2">
                      {files.map((f, i) => (
                        <li key={`${f.name}-${i}`} className="flex items-center gap-3 rounded-xl border border-border/50 bg-muted/20 px-4 py-2.5 transition-colors hover:border-border">
                          ...
                          <Button variant="ghost" size="icon-sm" className="text-muted-foreground hover:text-destructive" onClick={() => setFiles((prev) => prev.filter((_, idx) => idx !== i))}>
                            <Trash2 className="size-3.5" /><span className="sr-only">移除 {f.name}</span>
                          </Button>
                        </li>
                      ))}
                    </ul>
                ...
                <CardContent className="flex flex-col gap-5 pt-6">
                  <InputGroup>
                    ...
                    <InputGroupAddon align="inline-end">
                      <InputGroupButton variant="default" onClick={runSearch} disabled={searching} className="rounded-full">
                  ...
                  <div className="flex items-center justify-between border-t border-border/50 pt-4 text-[11px] text-muted-foreground">
                    ...
                    <span className="font-mono">avg similarity {avgScore.toFixed(2)}</span>
                  </div>
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant="secondary" className={cn(
                                ...
                                r.similarity_score >= 0.9 ? 'bg-emerald-50 text-emerald-700' : r.similarity_score >= 0.82 ? 'bg-violet-50 text-violet-700' : 'bg-muted text-muted-foreground',
                              )}>
                            ...
                            <div className="mt-3 h-[3px] overflow-hidden rounded-full bg-muted">
                              <div className="h-full rounded-full bg-gradient-to-r from-violet-400 to-emerald-400" style={{ width: `${r.similarity_score * 100}%` }} />
                            </div>
                          </SheetTrigger>
                          <SheetContent side="right" className="w-[420px] sm:max-w-[480px]">
                            <SheetHeader>
                              <SheetTitle className="text-[15px]">片段详情</SheetTitle>
                              <SheetDescription>{r.chunk_id} · {r.source}</SheetDescription>
                            </SheetHeader>
          ...
          <div>
            <div className="mb-4">
              <SectionHeader icon={MessageSquare} title="RAG 对话" description="基于知识库的智能问答，回答附带可点击引用" />
            </div>
            <Card className="card-shadow flex flex-col bg-white xl:sticky xl:top-[104px] xl:h-[calc(100svh-136px)]">
                ...
                {messages.map((m) => (
                  <div key={m.id} className={cn('flex gap-3', m.role === 'user' ? 'justify-end' : 'justify-start')}>
                    ...
                    <div className={cn(
                      ...
                      m.role === 'user' ? 'rounded-br-md bg-primary/10 text-foreground ring-1 ring-primary/20' : 'rounded-bl-md border border-border/50 bg-muted/20',
                    )}>
                ...
                {thinking && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="flex items-center gap-2 text-[12px] text-muted-foreground">
                      <Spinner className="size-3.5" />正在检索知识库并生成引用…
                    </span>
                  </div>
                )}
              </CardContent>
              ...
              <div className="p-4">
                <InputGroup>
                  ...
                  <InputGroupAddon align="block-end">
                    ...
                    <InputGroupButton variant="default" className="ml-auto rounded-full" onClick={send} disabled={thinking || !draft.trim()}>
                      <SendHorizontal data-icon="inline-start" />发送
                    </InputGroupButton>
                  </InputGroupAddon>
                </InputGroup>
              </div>
...
/* ── MetricChip (for hero area) ── */
function MetricChip({ icon: Icon, label, value }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string }) {
  return (
    <div className="flex flex-col items-center gap-1 rounded-xl bg-white/80 p-3 backdrop-blur-sm ring-1 ring-black/5">
