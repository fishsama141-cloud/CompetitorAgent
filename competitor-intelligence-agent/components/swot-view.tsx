...
const TONE: Record<string, { chip: string; bar: string }> = {
  strength: { chip: 'bg-violet-50 text-violet-600 ring-violet-200', bar: 'bg-violet-400' },
...
export function SwotView({ domain }: { domain: string }) {
  ...
  async function generate() {
    ...
    try {
      ...
      toast.success('SWOT 报告已生成', { description: `${label} · 近 ${days} 天 · report_id ${result.report_id}` })
    } catch {
      await new Promise((r) => setTimeout(r, 1400))
      const fakeId = `rpt_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}_swot`
      setLatestReportId(fakeId)
      toast.success('SWOT 报告已生成（模拟）', { description: `${label} · 近 ${days} 天 · report_id ${fakeId}` })
    } finally {
      setGenerating(false)
    }
  }
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white via-violet-50/60 to-violet-100/40 px-6 py-10 lg:px-10 lg:py-14">
        <div className="relative z-10 flex flex-wrap items-start justify-between gap-6">
          <div className="flex flex-col gap-2">
            <span className="font-mono text-[10px] font-medium tracking-[0.2em] text-primary/60 uppercase">Strategy</span>
            <h1 className="text-[26px] font-bold leading-tight tracking-tight lg:text-[32px]">SWOT 战略分析</h1>
            <p className="max-w-lg text-[13px] leading-relaxed text-muted-foreground">
              基于知识库生成带引用的竞争态势矩阵。选择对比目标与时间窗口，由智能体自动推导优劣势与战略建议。
            </p>
          </div>
          <Button onClick={generate} disabled={generating || !targets.length} size="lg" className="rounded-full">
            {generating ? <Spinner data-icon="inline-start" /> : <Zap data-icon="inline-start" />}
      ...
      <section className="mt-8">
        <div className="mb-4">
          <SectionHeader icon={Sparkles} title="报告配置" description="选择对比目标与时间窗口" />
        </div>
      ...
      {/* ================================================================
          SECTION 3 — 4 Quadrants
          ================================================================ */}
      <section className="mt-8">
        <div className="mb-4">
          <SectionHeader icon={Shield} title="竞争态势矩阵" description="4 象限带可点击引用来源" />
        </div>
        <div className="grid gap-5 lg:grid-cols-2">
          {QUADRANTS.map((q) => {
            ...
            return (
              <Card key={q.key} className="card-shadow gap-0 bg-white py-0 transition-all hover:card-shadow-hover">
                <div className="relative flex items-center gap-3 px-5 py-5">
                  <div className={cn('flex size-9 items-center justify-center rounded-xl ring-1', tone.chip)}>
                    <Icon className="size-4" />
                  </div>
                  <div className="flex min-w-0 flex-col">
                    <span className="truncate text-[14px] font-semibold tracking-tight">{q.title}</span>
                    <span className="font-mono text-[10px] text-muted-foreground">{items.length} insights · cited</span>
                  </div>
                  <span className={cn('ml-auto rounded-lg px-2 py-1 font-mono text-[10px] font-medium ring-1', tone.chip)}>
                    {avgConf}% avg
                  </span>
                </div>
                ...
                <div className="relative flex flex-col">
                  {items.map((item, i) => (
                    <SwotRow key={`${q.key}-${i}`} item={item} tone={q.tone} last={i === items.length - 1} />
                  ))}
                </div>
              </Card>
            )
      ...
      <section className="mt-8">
        <Card className="card-shadow bg-white">
          <CardHeader className="cursor-pointer border-b border-border/50 pb-5 hover:bg-muted/20" onClick={() => setShowRecs(!showRecs)}>
            <div className="flex flex-wrap items-start gap-4">
              <div className="flex min-w-0 flex-1 items-center gap-2.5">
                ...
                <div>
                  <CardTitle className="text-[15px] tracking-tight">战略建议</CardTitle>
              ...
              <div className="flex items-center gap-2">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger render={<Button variant="outline" size="sm" className="rounded-full" onClick={(e) => e.stopPropagation()} />}>
                    <Download data-icon="inline-start" className="size-3.5" />导出
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => toast('正在导出 PDF…')}><FileDown className="size-3.5" />PDF</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => toast('正在导出 Markdown…')}><Download className="size-3.5" />Markdown</DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
            ...
            <CardContent className="pt-6">
              <ul className="flex flex-col gap-3">
                {recommendations.map((rec, i) => (
                  <li key={rec} className="flex gap-4 rounded-2xl border border-border/50 bg-muted/15 p-5 transition-all hover:border-primary/20 hover:bg-primary/[0.02]">
    ...
    <div className={cn('flex flex-col gap-2.5 px-5 py-4 transition-colors hover:bg-muted/20', !last && 'border-b border-border/40')}>
      ...
      <div className="flex items-center gap-2 pl-[18px]">
        ...
        <div className="ml-auto hidden w-20 shrink-0 overflow-hidden rounded-full bg-muted sm:block">
          <div className={cn('h-[3px] rounded-full', t.bar)} style={{ width: `${item.confidence * 100}%` }} />
        </div>
      </div>
    </div>
  )
